...
// ---------------------------------------------------------------------------
// § 8  pre-02: {{stage.items_from}} substitution
// ---------------------------------------------------------------------------

describe('assemblePrompt — pre-02 {{stage.items_from}}', () => {
  it('{{stage.items_from}} resolves to the passed-in path string', () => {
    const ctx: PromptContext = {
      stage: { items_from: 'docs/idea/round-2-features.json' },
    };
    expect(assemblePrompt('Read `{{stage.items_from}}`', ctx)).toBe(
      'Read `docs/idea/round-2-features.json`',
    );
  });

  it('{{stage.items_from}} is "" when not set → empty substitution', () => {
    const ctx: PromptContext = {
      stage: { items_from: '' },
    };
    expect(assemblePrompt('path: {{stage.items_from}}', ctx)).toBe('path: ');
  });

  it('{{stage.items_from}} missing entirely → [MISSING:stage.items_from]', () => {
    expect(assemblePrompt('{{stage.items_from}}', {})).toBe('[MISSING:stage.items_from]');
  });
});
