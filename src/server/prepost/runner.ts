...
export type RunCommandsResult =
  ...
  | { kind: 'action'; command: string; action: ActionValue }
  /** A command exceeded its wall-clock timeout. */
  | { kind: 'timeout'; command: string }
  /** A command could not be spawned (ENOENT, bad run array, etc.). */
  | { kind: 'spawn_failed'; command: string; error: Error }
  /** A command exited with a code not declared in its actions map. */
  | { kind: 'unhandled_exit'; command: string; exitCode: number };
 ...
 * All log frames are flushed before returning.
 *
...
export async function runCommands(opts: RunCommandsOpts): Promise<RunCommandsResult> {
  for (const cmd of opts.commands) {
    const result = await _runOneCommand(cmd, opts);
    if (result.kind !== 'continue_next') {
      return result;
    }
  }
...
/** Internal result — adds 'continue_next' sentinel for the loop. */
...
async function _runOneCommand(
  ...
  if (outcome.type === 'spawn_error') {
    return { kind: 'spawn_failed', command: cmd.name, error: outcome.err };
  }
  ...
  if (outcome.type === 'timeout') {
    ...
    await _writeQueue;
    return { kind: 'timeout', command: cmd.name };
  }
  ...
  if (action === null) {
    return { kind: 'unhandled_exit', command: cmd.name, exitCode };
  }

  if (isContinue(action)) {
    return { kind: 'continue_next' };
  }

  return { kind: 'action', command: cmd.name, action };
}
