...
const ALL_SEVERITIES = ['info', 'warn', 'error', 'requires_attention'] as const;
...
function writeNoticesLog(logPath: string, sessionId: string, wfId: string): void {
  const lines = [
    JSON.stringify({ type: 'session.started', sessionId, workflowId: wfId, ts: '2026-01-01T00:00:00Z', seq: 0, payload: { sessionId, phase: 'implement', attempt: 1, startedAt: '2026-01-01T00:00:00Z', parentSessionId: null } }),
    ...ALL_SEVERITIES.map((severity, i) =>
      JSON.stringify({ type: 'stream.system_notice', sessionId, workflowId: wfId, ts: '2026-01-01T00:01:00Z', seq: i + 1, payload: { blockId: `notice-${severity}`, severity, source: 'test', message: `Notice severity ${severity}` } }),
    ),
  ].join('\n') + '\n';
  fs.writeFileSync(logPath, lines);
}

function seedItemAndSession(
  db: BackendHandle['db'],
  opts: { wfId: string; itemId: string; sessionId: string; logPath: string },
): void {
  const now = new Date().toISOString();
  db.writer
    .prepare(`INSERT INTO items (id, workflow_id, stage_id, data, status, updated_at) VALUES (?, ?, 'stage-1', '{}', 'complete', ?)`)
    .run(opts.itemId, opts.wfId, now);
  db.writer
    .prepare(`INSERT INTO sessions (id, workflow_id, item_id, stage, phase, agent_profile, started_at, ended_at, exit_code, status, session_log_path) VALUES (?, ?, ?, 'stage-1', 'implement', 'default', '2026-01-01T00:00:00Z', '2026-01-01T01:00:00Z', 0, 'complete', ?)`)
...
test.describe('stream-pane regressions — real backend', () => {
  test('B1: system notices of all severities render without crashing', async ({
    ...
    try {
      writeNoticesLog(logPath, sessionId, wfId);
      seedWorkflow(backend.db, { id: wfId, name: 'Stream-Pane B1 Workflow', status: 'completed' });
      seedItemAndSession(backend.db, { wfId, itemId, sessionId, logPath });
  ...
  test('A7: cancel button appears for in_progress workflow with active session', async ({
    ...
    const now = new Date().toISOString();
    backend.db.writer
      .prepare(`INSERT INTO items (id, workflow_id, stage_id, data, status, updated_at) VALUES (?, ?, 'stage-1', '{}', 'in_progress', ?)`)
      .run(`item-sp-a7-${randomUUID().slice(0, 8)}`, wfId, now);
    // Active session: ended_at IS NULL so it appears in activeSessions.
    backend.db.writer
      .prepare(`INSERT INTO sessions (id, workflow_id, item_id, stage, phase, agent_profile, started_at, status) VALUES (?, ?, NULL, 'stage-1', 'implement', 'default', ?, 'in_progress')`)
