...
test('stage-history: past session log renders in History tab', async ({ page, backend }) => {
  ...
  // Create a temp dir for the JSONL log file outside the fixture's .yoke/ dir.
  const logDir = fs.mkdtempSync(path.join(os.tmpdir(), 'yoke-hist-log-'));
  const logPath = path.join(logDir, `${sessionId}.jsonl`);

  try {
    // Write two frames: session.started so the store initialises the session,
    // then stream.system_notice with recognisable text for assertion.
    const logLines = [
      JSON.stringify({
      ...
      JSON.stringify({
        ...
        payload: {
          ...
          severity: 'info',
          source: 'test',
          message: 'History test content',
        },
      }),
    ].join('\n') + '\n';
    fs.writeFileSync(logPath, logLines);

    // Seed DB: workflow, item, and completed session with log path.
    const pipeline = JSON.stringify({
      stages: [{ id: 'stage-1', run: 'once', phases: ['implement'] }],
    });
    backend.db.writer
      ...
      .run(wfId, 'History Test Workflow', pipeline, now, now);
    ...
    // Item must be visible in the FeatureBoard before clicking.
    await expect(page.locator(`#item-${itemId}`)).toBeVisible({ timeout: 5000 });

    // Click item — triggers GET /api/items/:id/sessions.
    await page.locator(`#item-${itemId}`).click();

    // History tab must appear and become enabled (1 session seeded).
    const historyTab = page.getByTestId('tab-history');
    await expect(historyTab).toBeVisible();
    ...
    // The past session row must be visible in the session list.
    const sessionRow = page.getByTestId(`history-session-${sessionId}`);
    await expect(sessionRow).toBeVisible();

    // Click session — triggers log load then renders LiveStreamPane.
    ...
    // Live pane shows "No active session" since workflow is complete.
