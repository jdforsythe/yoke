...
## dashboard-features — implement attempt 6 (2026-04-15)

Fixed two spec-compliance violations that had been deferred across attempts 1–5. **prepost.command.output rAF batching** (`WorkflowDetailRoute.tsx`): the frame handler for `prepost.command.output` was routed through `dispatch()` (immediate), but `feat-prepost-rendering` RC-2 / AC-5 require output chunks to be "buffered and flushed at 16ms intervals (same rAF batching as text deltas)". Moving the handler to `dispatchTextDelta()` places chunks in the same rAF queue as `stream.text` and `stream.thinking` frames, satisfying the spec without any reducer changes. **item.data cache clear on navigation** (`FeatureBoard.tsx`, `WorkflowDetailRoute.tsx`): the module-level `itemDataCache` Map was never cleared when navigating between workflows — only per-item invalidation on `item.state` frames was implemented. Added `clearItemDataCache()` export to `FeatureBoard.tsx` and called it from `WorkflowDetailRoute`'s cleanup effect alongside `reset()`. Stale item data no longer persists across workflow navigations. `tsc -p tsconfig.json --noEmit` clean; `pnpm build` succeeds with zero warnings; all 32 Playwright e2e smoke tests pass in ~4.2s.

## dashboard-features — implement attempt 5 (2026-04-15)

Addressed three review-criteria violations identified in the prior attempt. **Session-specific snapshot selector** (`renderStore.ts`): added `getSessionBlocksSnapshot(sessionId)` which maintains a per-session cache keyed on the `BlockRing` reference — every `pushBlock`/`updateBlock` creates a new ring via `clone()`, so a changed ring reference is the staleness signal. When the ring is unchanged, the same `readonly RenderBlock[]` array reference is returned, causing `useSyncExternalStore` (via `Object.is`) to skip re-renders. `reset()` clears the cache on workflow navigation. **LiveStreamPane + ReviewPanel** (`LiveStreamPane.tsx`, `ReviewPanel.tsx`): both components now subscribe with `useSyncExternalStore(subscribe, () => getSessionBlocksSnapshot(sessionId))` instead of the full-model `getSnapshot`. This satisfies `feat-stream-pane` RC6 and `feat-review-panel` RC6 ("the component does not subscribe to the full render-model state; it receives blocks for the active session only") — frames from unrelated sessions, usage updates, and non-block frames no longer trigger re-renders in these components. **AppShell toast portal** (`AppShell.tsx`): moved the push-notification fallback toast stack from an inline `<div>` at the end of the AppShell JSX to `createPortal(…, document.body)` inside a React `<>…</>` Fragment, satisfying `feat-service-worker-push` RC4 ("Toast fallback renders via a portal-based stack, not alert()"). `tsc --noEmit` clean; `pnpm build` succeeds with zero warnings; all 32 Playwright e2e smoke tests pass in ~3.7s.

## dashboard-features — implement attempt 4 (2026-04-14)

Addressed four review-criteria violations identified in the prior review cycle. **Ring buffer eviction (O(1))**: replaced `pushBlock`'s `Array.shift()` + `Array.unshift()` calls (both O(n)) with a new `BlockRing` class (`store/blockRing.ts`) that uses head-pointer advancement for O(1) eviction — no physical element moves. `SessionRenderState` no longer exposes a flat `blocks` array; live blocks are materialised on demand by `getSessionBlocks()` via `ring.toArray()`, with a synthetic `TruncatedSentinel` prepended when `_evictedCount > 0`. **ToolCall O(1) Map lookup**: `SessionRenderState` gains `_blockMap: Map<blockId, physIdx>` that is populated on every `pushBlock` and used by `stream.tool_result` (and text/thinking delta accumulation) for O(1) lookup by `toolUseId`/`blockId` — no `findIndex` linear scan. **UsageHUD selective subscription**: `renderStore` now exposes `getUsageSnapshot()` that returns a stable object reference unless token totals change; `UsageHUD` subscribes with `useSyncExternalStore(subscribe, getUsageSnapshot)` so it only re-renders on `stream.usage` frames, not on every text delta. **UsageHUD portal dropdown**: the usage breakdown dropdown is now rendered via `createPortal(…, document.body)` with `fixed` positioning computed from `getBoundingClientRect`, satisfying the "positioned with a portal to avoid overflow clipping" review criterion. `tsc --noEmit` clean; `pnpm build` succeeds with zero warnings; all 32 Playwright e2e smoke tests pass in ~3.6s.

## dashboard-features — implement attempt 3 (2026-04-14)

Addressed two spec-compliance gaps identified during thorough post-attempt-2 review. **ReviewPanel phase prop**: added explicit `phase: string` to the Props interface so the activation contract is expressed in the type system, not only in comments; `WorkflowDetailRoute` now passes `activeSessionPhase!` when rendering `ReviewPanel`, keeping phase detection anchored to `SessionStartedPayload.phase` as required by the spec. **FeatureBoard deep-link scroll guard**: changed the deep-link scroll `useEffect` to `useLayoutEffect` so `scrollIntoView` fires synchronously after DOM mutations and before the browser paints — eliminating the theoretical flash-of-unstyled-content on cold-start deep links that the spec explicitly requires a "layout-effect guard" to prevent. `tsc --noEmit` clean; `pnpm build` succeeds with zero warnings; all 32 Playwright e2e smoke tests pass in ~3.7s.

## dashboard-features — implement attempt 2 (2026-04-14)

Fixed two deferred bugs and added 32 Playwright e2e smoke tests (all passing). **Bug fixes**: `FeatureBoard` now exports `invalidateItemData(itemId)` which deletes the item from the module-level `itemDataCache`; `WorkflowDetailRoute` calls it inside the `item.state` frame handler so re-selecting an item after a state change triggers a fresh HTTP fetch instead of serving stale data. The deep-link scroll `useEffect` in `FeatureBoard` now includes `items` in its dependency array, fixing the race where the snapshot arrives asynchronously after mount — previously `itemRefs.current` was empty at mount time and the scroll-to silently dropped. **Playwright e2e smoke tests** (`pnpm test:e2e`): added `playwright.config.ts` targeting `vite preview --port 4173`; three spec files with shared WS/API mock helpers (`e2e/helpers.ts`). `app-shell.spec.ts` (5 tests): brand render, connection indicator transitions (Connected / Connecting / Version mismatch), WorkflowListRoute placeholder. `workflow-list.spec.ts` (7 tests): empty state, rows with name + status chip + unread badge, filter bar, row-click navigation, active-row `aria-current`, real-time `workflow.index.update` patch. `workflow-detail.spec.ts` (20 tests): loading state, workflow header, back nav; CrashRecoveryBanner (non-null recovery, uncommittedChanges, confirmation dialog); AttentionBanner (items present, hidden when empty); GithubButton (all six status values); FeatureBoard (grouped items, fuzzy search, retryCount badge); ControlMatrix (Pause/Cancel/Inject for in_progress, Cancel confirmation dialog, Resume for paused, hidden without active session). All 32 pass in ~4s against the production build.

## dashboard-features — implement attempt 1 (2026-04-14)

Implemented the full Yoke web dashboard in five commits under `src/web/src/`. Prior commits (scaffold, WS client, render reducer) were already present; this session built every remaining feature in topological order. **Service worker & store** (`sw.ts`, `renderStore.ts`, `useRenderModel.ts`): minimal SW for push notifications; module-level observable render store wrapping the pure reducer with rAF-batched text-delta dispatch via `dispatchTextDelta()`; `useSyncExternalStore` hook. **UsageHUD & WorkflowList**: UsageHUD reads `getTotalUsage` from the render store, abbreviates with k/M formatting, and expands a per-session breakdown dropdown. WorkflowList fetches via keyset pagination (`before=createdAt`), patches rows in-place on `workflow.index.update` frames, and debounces search with `AbortController`-cancellable in-flight fetches; IntersectionObserver drives infinite scroll; filter params reflected in URL query string. **LiveStream block renderers & LiveStreamPane**: TextBlockRenderer parses fenced code blocks without `dangerouslySetInnerHTML`; ThinkingBlockRenderer collapses by default with local expand state; ToolCallRenderer shows input/output JSON in a collapsible; SystemNoticeRenderer has severity-coloured left-rule accent with prepost output log. LiveStreamPane uses `@tanstack/react-virtual` with `measureElement`, follow-tail auto-scroll, detach-on-upscroll with "Jump to latest" pill, and session scroll-position cache. **Banners & board**: CrashRecoveryBanner is undismissable without server ack; AttentionBanner does optimistic removal via POST .../ack with revert on failure; GithubButton handles all six `GithubState.status` values exhaustively; FeatureBoard groups items by stageId using a `Map`, implements fuzzy search, j/k keyboard nav via `aria-activedescendant`, deep-link scroll+highlight, and on-demand `item.data` HTTP fetch with client-side cache; ReviewPanel partitions `name === 'Task'` tool_use blocks into collapsible subagent rows with pass/fail summary header. **ControlMatrix & routes**: ControlMatrix derives visible actions from a declarative `RULES` record, uses controlled confirmation dialogs for destructive actions, and the inject-context modal; WorkflowDetailRoute wires all WS frame types to the render store and composes all child components; ItemDetailRoute delegates to WorkflowDetailRoute with FeatureBoard reading `:itemId` for deep-link. TypeScript strict-mode clean (`tsc --noEmit` exit 0); production build succeeds with zero warnings (`pnpm build` in `src/web/`).
