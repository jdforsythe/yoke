...
1. Read the feature spec above. Note every AC and RC.
...
3. **Do NOT modify `docs/brainstorm-features.json`.** That file is the manifest; the planner owns it. Any change trips the diff check and resets your progress.
4. Sketch the smallest implementation that satisfies every AC. Do not gold-plate.
5. Write code in small commits — no more than five files per commit.
6. Every code path that can fail gets at least one test.
