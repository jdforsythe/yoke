...
1. Read the feature spec above. Note the acceptance criteria (AC) and review criteria (RC).
...
6. If the spec is genuinely ambiguous, document the ambiguity in the handoff entry and pick the simplest interpretation. Do not block.
...
cat <<'JSON' | node scripts/append-handoff-entry.js
{
  ...
  "known_risks": ["<risks for the next reader>"]
}
JSON
```
