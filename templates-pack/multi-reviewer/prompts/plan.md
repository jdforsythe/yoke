...
You are decomposing the brief at `docs/idea/multi-reviewer-brief.md` into a topologically-ordered features manifest for workflow **{{workflow_name}}** (stage `{{stage_id}}`).
...
1. Read `docs/idea/multi-reviewer-brief.md` end-to-end. Identify goal, hard constraints, explicit non-goals, and any acceptance criteria already stated. Resist the urge to redesign — you are decomposing, not improving.
...
6. When a criterion is anchored in the brief, cite it (e.g. "per docs/idea/multi-reviewer-brief.md §Auth").

If the brief is too vague to produce a complete manifest:
1. Write a partial `docs/idea/multi-reviewer-features.json` with the features you are confident about.
...
Write `docs/idea/multi-reviewer-features.json`:

```json
{
  "project": "<workflow name>",
  "created": "<ISO 8601 UTC timestamp>",
  "source": "docs/idea/multi-reviewer-brief.md",
  "features": [
    {
      "id": "feat-<kebab-slug>",
